...
Order Details:
${order.items.map((item: any) => `• ${item.productName} (Qty: ${item.quantity})`).join('\n')}
          ...
          <ul>
            ${order.items.map((item: any) => `
              <li>${item.productName} - Quantity: ${item.quantity}
                ${item.customization?.text ? `<br><small>Customization: ${item.customization.text}</small>` : ''}
              </li>
            `).join('')}
          </ul>
        </div>
...
${newStatus === 'shipped' ? `Track your package and expect delivery within 2-3 days.` : ''}
${newStatus === 'delivered' ? `Thank you for choosing Handcrafted Gifts! We'd love to see how you're enjoying your purchase. 📸` : ''}
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 30px; background: white;">
        ...
        ${newStatus === 'shipped' ? '<p>Your package is on its way! You can expect delivery within 2-3 days.</p>' : ''}
        ${newStatus === 'delivered' ? '<p>We hope you absolutely love your handcrafted item! If you\'re happy with your purchase, we\'d love to see a photo. 📸</p>' : ''}
